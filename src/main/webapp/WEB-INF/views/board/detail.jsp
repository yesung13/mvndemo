...
<head>
<title>글보기:${boardDetail.title}</title>
<style>
.uploadResult {
	width: 100%;
...
/*        function replyWrite_btn() {
        ...
        let replyUpdate_form = function (boardId, replyId, replyWriter, replyContent) {
            ...
            if (replyId != null || replyId !== "" || replyId !== undefined) {
                var html = "";
                html += "<th class='text-center'>" + "<div class='pt-2'>" + "<strong class='text-muted'>" + replyWriter + "</strong>" + "</div>" + "</th>"
                html += "<th>"
		...
		<div class="container">
			<table class="table table-borderless">
				<tr class="text-center thead-light">
					<th colspan="4">
						<h4 class="font-weight-bold" id="title">${boardDetail.title}</h4>
					</th>
				</tr>
				<tr class="border-bottom">
					...
					<td class="text-left">
						<span id="writer">${boardDetail.writer}</span>
					</td>
				</tr>
				<tr class="border-bottom">
					<th class="text-center">
				...
				<tr class="text-left">
					<td colspan="4" style="height: 200px; min-height: 200px; max-height: 500px; padding: 30px 50px;">
						<div id="content" style="font-size: 1.2em;">${boardDetail.content}</div>
					</td>
				</tr>
				...
				<tr class="text-right thead-light border-bottom">
					<td colspan="4">
						<span>
							<small class="text-muted font-weight-bold" id="boardDatetime">${boardDetail.writeDt}</small>
						</span>
					</td>
				</tr>
			</table>
 	...
 	<section>
		<div class="container">
				...
				<tbody>
					...
					<tr class="text-center" id="changeUpdateForm">
						<form id="boardDetailForm">
							...
							<th>
								<div class="pt-2">
									<strong id="replyWriter" class="text-muted">댓글 작성자</strong>
								</div>
							</th>
							<th>
								<div class="form-inline">
									<textarea id="replyContent" name="replyContent" rows="1" class="form-control rounded w-75" placeholder="댓글을 남겨보세요"></textarea>
						...
						<c:when test="${fn:length(replyList) eq 0 || fn:length(replyList) eq null}">
							<tr>
								<td class="text-center" colspan="2">
									<span class="text-black-50">댓글이 없습니다.</span>
								</td>
							</tr>
						</c:when>
						<c:otherwise>
							<c:forEach var="item" items="${replyList}">
								<tr>
									<th class="text-center">
										<div>
											<strong>${item.replyWriter}</strong>
										</div>
									</th>
									<th>
										<c:if test="${sessionScope.account.id eq item.userId}">
											...
											<div class="dropdown">
												...
												<div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownBtn" style="width: 100px">
													...
													<button class="dropdown-item" type="button" onclick="replyDelete_btn('${item.replyId}',${item.boardId})">
														<small>삭제</small>
													</button>
												</div>
											</div>
										...
										<div class="mt-3">
											<small class="text-muted">${item.replyDatetime}</small>
										</div>
									</th>
								</tr>
							</c:forEach>
			...
			<div class="d-flex justify-content-end">
				<div class="mt-2">
					<c:choose>
						<c:when test="${sessionScope.account.id eq boardDetail.boardId}">
